    ...
    <div className="min-h-screen bg-surface dark:bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 transition-colors duration-300">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="flex justify-center flex-col items-center">
           <img src={logoUrl} alt="Curexal Logo" className="h-16 w-16 rounded-xl object-cover shadow-sm border border-slate-200 dark:border-slate-800" />
        </div>
        <h2 className="mt-6 text-center text-3xl font-extrabold text-foreground dark:text-slate-100">
          Create new password
        </h2>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-background dark:bg-slate-950 py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-border dark:border-slate-800 transition-colors duration-300">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            {!token && (
               <div className="bg-red-50 dark:bg-red-900/30 border-l-4 border-red-400 dark:border-red-500 p-4 rounded-md flex items-start mb-6">
                 <AlertCircle className="h-5 w-5 text-red-500 dark:text-red-400 mr-2 shrink-0 mt-0.5" />
                 <p className="text-sm text-red-700 dark:text-red-300">Missing password reset token in URL.</p>
               </div>
            )}
            ...
            {successMsg && (
              <div className="bg-accent-50 dark:bg-accent-900/30 border-l-4 border-accent-400 dark:border-accent-500 p-4 rounded-md flex items-start">
                <CheckCircle className="h-5 w-5 text-accent-600 dark:text-accent-400 mr-2 shrink-0 mt-0.5" />
                <p className="text-sm text-accent-800 dark:text-accent-300">{successMsg}</p>
              </div>
            )}
            ...
            <div>
              ...
              <div className="mt-1 relative rounded-md shadow-sm">
                ...
                <input
                  ...
                  className="focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 sm:text-sm border-border dark:border-slate-700 rounded-md py-2 px-3 border bg-surface dark:bg-slate-900 text-foreground dark:text-slate-100 focus:outline-none disabled:opacity-50"
                  placeholder="••••••••"
                />
              </div>
            </div>
